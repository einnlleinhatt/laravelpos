    ...
    private function completeReturnOrder($original_return_order, $type)
    {
        ...
        if (!$original_return_order || $original_return_order->draft) {
            if ($this->return_order->customer_id) {
                if (safe_email($this->return_order->customer->email) && !default_customer($this->return_order->customer_id)) {
                    $this->return_order->customer->notify(new \Modules\MPS\Notifications\ReturnCreated($this->return_order));
                }
            }
            if ($this->return_order->supplier_id) {
                if (safe_email($this->return_order->supplier->email)) {
                    $this->return_order->supplier->notify(new \Modules\MPS\Notifications\ReturnCreated($this->return_order));
                }
            }
        }
        ...
        foreach ($this->return_order->items as $item) {
            ...
            if ($stock) {
                ...
                // DB::transaction(function () use ($stock, $base_quantity, $return_order) {
                // $stock = $stock->fresh();
                $stock->update([
                    'quantity' => $return_order->type == 'sale' ? $stock->quantity + $base_quantity : $stock->quantity - $base_quantity,
                ]);
                // }, 3);
            }
        }

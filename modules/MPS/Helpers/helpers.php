...
if (!function_exists('generate_reference')) {
    function generate_reference($model)
    {
        ...
        if ($format == 'yearly') {
            return usermeta($model->user_id ?: auth()->id(), 'number') . '/' . now()->format('Y/') . sequence_number($model);
        } elseif ($format == 'monthly') {
            return usermeta($model->user_id ?: auth()->id(), 'number') . '/' . now()->format('Y/m/') . sequence_number($model);
        } elseif ($format == 'sequence') {
            return usermeta($model->user_id ?: auth()->id(), 'number') . '/' . sequence_number($model);
        }
        return usermeta($model->user_id ?: auth()->id(), 'number') . '/' . \Illuminate\Support\Str::random(12);
    }
}
...
if (!function_exists('mps_data')) {
    function mps_data()
    {
        $modules = collect(Module::collections()->toArray())->transform(function ($item) {

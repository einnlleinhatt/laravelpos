...
class PaymentReminder extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        $mail = (new MailMessage)
            ...
            ->subject(__('mps::mail.payment_reminder_subject'))
            ->line(__('mps::mail.payment_reminder_opening_line', ['created_at' => $this->payment->created_at->format(php_date_formate()), 'reference' => $this->payment->reference]))
            ->action(__('mps::mail.payment_reminder_button_text'), $url)
            ->line(__('mps::mail.payment_reminder_closing_line'));
